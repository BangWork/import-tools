...
func CheckProjectDiskAvailable(url, cookie string, h map[string]string, projectOriginalKeys []string) (bool, error) {
	...
	orgConfig, err := ones.GetOrgConfig(url, h)
	if err != nil {
		return false, err
	}
...
func GetONESDiskInfo(url, cookie string, h map[string]string) (resp *ones.ResolveResultResponse, err error) {
	importCache := common.ImportCacheMap.Get(cookie)
